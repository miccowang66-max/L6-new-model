...
| 2.3 | 2026-06-11 | Streamlit 儀表板（`app.py`）全面重新設計為 5 章節中文架構：一、專案概述與技術架構；二、資料預處理流程（原始資料 + One-Hot Encoding + StandardScaler + 互動式相關性熱力圖）；三、五大特徵選擇方法（共識表 + 每方法獨立 expander 含對應圖表：BE α 滑桿動態 P 值長條圖、FS R²/Adj.R² 步驟線圖、RFE 重要性排名、Lasso α 滑桿係數收縮路徑、MI 相依性分數）；四、模型優化與診斷（SFA 互動切換圖 + 統計診斷指標卡 + 效率轉折點 Elbow Point 雙圖分析）；五、關鍵發現（核心洞察色卡 → 特徵得票圖 → 最佳模型績效卡 → 預測 vs 實際散佈圖 → 商業決策指引三卡片）|
